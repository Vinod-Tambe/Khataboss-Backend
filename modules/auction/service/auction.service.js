...
class AuctionService {
  ...
  async addAuction(dbUrl, reqUser, data) {
    ...
    try {
      ...
      // Fetch original loan to get its accounts
      const originalGirvi = await prisma.girvi.findUnique({ where: { girv_id: parseInt(data.auc_girv_id) } });
      const loanDrAccId = originalGirvi?.girv_dr_acc_id || null;

      // Create Auction Loan (Transaction) Record
      const newAuctionLoan = await prisma.auctionLoan.create({
        data: {
          al_date: data.auc_date || new Date().toISOString().split('T')[0],
          al_girv_id: parseInt(data.auc_girv_id),
          al_firm_id: firmId,
          al_buyer_id: auctionUserId,

          al_prin_amt: parseFloat(data.auc_prin_amt) || 0,
          al_int_amt: parseFloat(data.auc_int_amt) || 0,
          al_dep_amt: parseFloat(data.auc_dep_amt) || 0,
          al_payable_amt: parseFloat(data.auc_payable_amt) || 0,
      ...
      // Add Journal Entries for Payments Received
      const journal_request = {
        journal_date: {
          jrnl_firm_id: firmId,
          jrnl_own_id: reqUser.own_id,
          jrnl_user_id: auctionUserId,
          jrnl_date: new Date().toISOString(),
          jrnl_amt: newAuctionLoan.al_payable_amt,
          jrnl_panel: "Auction",
          jrnl_other_info: `Auction Payment | Auc No - ${newAuctionLoan.al_id}`,
        },
        joural_trans_data: [
          { jrtr_crdr: "DR", jrtr_date: new Date().toISOString(), jrtr_dr_acc_id: newAuctionLoan.al_cash_acc_id, jrtr_dr_amt: newAuctionLoan.al_cash_amt, jrtr_acc_info: newAuctionLoan.al_cash_info },
          { jrtr_crdr: "DR", jrtr_date: new Date().toISOString(), jrtr_dr_acc_id: newAuctionLoan.al_bank_acc_id, jrtr_dr_amt: newAuctionLoan.al_bank_amt, jrtr_acc_info: newAuctionLoan.al_bank_info },
          { jrtr_crdr: "DR", jrtr_date: new Date().toISOString(), jrtr_dr_acc_id: newAuctionLoan.al_online_acc_id, jrtr_dr_amt: newAuctionLoan.al_online_amt, jrtr_acc_info: newAuctionLoan.al_online_info },
          { jrtr_crdr: "DR", jrtr_date: new Date().toISOString(), jrtr_dr_acc_id: newAuctionLoan.al_card_acc_id, jrtr_dr_amt: newAuctionLoan.al_card_amt, jrtr_acc_info: newAuctionLoan.al_card_info },
          { jrtr_crdr: "CR", jrtr_date: new Date().toISOString(), jrtr_cr_acc_id: loanDrAccId, jrtr_cr_amt: newAuctionLoan.al_payable_amt, jrtr_acc_info: `Auction Payment : Auc No - ${newAuctionLoan.al_id}` }
