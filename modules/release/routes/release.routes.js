...
router.post(
  "/",
  requirePermission("loan.release"),
  upload.fields([{ name: "other_images", maxCount: 10 }]),
  (req, res) => releaseController.addRelease(req, res)
);
router.get(
  "/users",
  requirePermission("loan.view"),
  (req, res) => releaseController.getReleaseUsers(req, res)
);
