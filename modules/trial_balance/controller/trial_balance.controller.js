...
const trialBalanceService = require("../service/trial_balance.service");
...
class TrialBalanceController {
   ...
   * GET /trial-balance
   */
  async get_all_trial_balance_entries(req, res) {
    try {
      ...
      const response = await trialBalanceService.get_all_trial_balance_data(dbUrl, filters);

      return res.status(200).json({
        success: true,
        message: "Trial balance entries fetched successfully",
        data: response,
      });
    } catch (error) {
      ...
      return res.status(500).json({
        success: false,
        message: "Failed to fetch trial balance entries",
        error: error.message,
      });
    }
  }
