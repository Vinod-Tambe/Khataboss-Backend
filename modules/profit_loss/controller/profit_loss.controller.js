...
const profitLossService = require("../service/profit_loss.service");
...
class ProfitLossController {
   ...
   * GET /profit-loss
   */
  async get_all_profit_loss_entries(req, res) {
    try {
      ...
      const response = await profitLossService.get_profit_loss_data(dbUrl, filters);

      return res.status(200).json({
        success: true,
        message: "Profit & Loss entries fetched successfully",
        data: response,
      });
    } catch (error) {
      console.error("Error fetching profit & loss entries:", error);
      return res.status(500).json({
        success: false,
        message: "Failed to fetch profit & loss entries",
        error: error.message,
      });
    }
  }
